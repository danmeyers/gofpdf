...
func ExampleRegister() {
	...
	if err == nil {
		...
		barcode.Barcode(pdf, key, 15, 15, &width, &height, false)
	}
...
func TestBarcode(t *testing.T) {
	...
	var height float64 = 10
	barcode.Barcode(pdf, key, 15, 15, &width, &height, false)
	barcode.Barcode(pdf, key, 15, 35, nil, &height, false)
	barcode.Barcode(pdf, key, 15, 55, &width, nil, false)
	barcode.Barcode(pdf, key, 15, 75, nil, nil, false)
...
func TestGetUnscaledBarcodeDimensions(t *testing.T) {
	...
	key := barcode.RegisterQR(pdf, "qrcode", qr.H, qr.Unicode)
	barcode.Barcode(pdf, key, 15, 15, nil, nil, false)
